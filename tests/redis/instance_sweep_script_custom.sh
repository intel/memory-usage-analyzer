...
#
# This variant hardens the workload-kill path used by the OOM/liveness/timeout
# monitor. The original bare `kill "$pid"` only terminated the mb_start.sh shell,
# orphaning its memtier_benchmark clients (which live OUTSIDE the redisbench
# cgroup and so are not reaped by cgroup.kill). Those leaked clients polluted the
# next sweep point. See kill_workload_tree() below.
...
oom_kill_checks=1
phase_timeout=1800
# Grace period (seconds) between SIGTERM and SIGKILL when tree-killing a workload.
kill_grace=5
core_frequency="3500"
zram_mem_limit=""
zram_disk_size=""
...
print_usage() {
    cat <<'EOF_HELP'
Usage: instance_sweep_script_custom.sh [options]
...
Named options:
  ...
  --kill-grace <sec>                  Seconds to wait after SIGTERM before SIGKILL
                                      when tree-killing a stuck workload (default: 5)
...
# Echoes one PID per line. Deepest-first ordering means children are listed
# before their parents, so signalling in this order terminates leaves before
# their reapers get a chance to be re-parented under init.
collect_process_tree() {
    local root="$1"
    local -a children
    # `pgrep -P` lists direct children of $root.
...
# Gracefully terminate a workload process and its ENTIRE descendant tree.
#
# Why this exists: mb_start.sh launches memtier_benchmark (via numactl) as
# background children and installs no signal trap. A bare `kill` on the
# mb_start.sh shell therefore orphans those clients, which keep hammering the
# Redis servers. Because the clients are started by the sweep script (NOT moved
# into the redisbench cgroup), cgroup.kill cannot reap them either. The leaked
# clients then corrupt the following sweep point.
#
# Strategy (reasonable, escalating):
#   1. SIGTERM the whole tree, leaves first, so processes can exit cleanly.
#   2. Wait up to $kill_grace seconds for the tree to drain.
#   3. SIGKILL any survivors (re-scanning to catch late-spawned children).
kill_workload_tree() {
    local root="$1"
    [[ -z "$root" ]] && return 0
    ...
    # Phase 1: polite SIGTERM, deepest-first.
    local pid
    for pid in "${tree[@]}"; do
    ...
    # Phase 3: force-kill any survivors. Re-scan because grandchildren may have
    # been spawned after the first snapshot.
    mapfile -t tree < <(collect_process_tree "$root")
    for pid in "${tree[@]}"; do
...
# All kills go through kill_workload_tree so the memtier_benchmark clients are
# reaped along with the mb_start.sh shell.
    ...
    (
        ...
        while kill -0 "$target_pid" 2>/dev/null; do
            ...
            if (( new_kills >= oom_kill_checks )); then
                echo "WARNING: ${new_kills} new OOM kills detected (threshold=${oom_kill_checks}) — killing workload tree $target_pid"
                kill_workload_tree "$target_pid"
                break
            ...
            if (( alive == 0 )); then
                echo "WARNING: zero Redis servers responding — killing workload tree $target_pid"
                kill_workload_tree "$target_pid"
                break
            fi
            ...
            if (( elapsed >= phase_timeout )); then
                echo "WARNING: phase timeout ${phase_timeout}s exceeded (${elapsed}s elapsed) — killing workload tree $target_pid"
                kill_workload_tree "$target_pid"
                break
            fi
        done
...
# Also reaps any stray memtier_benchmark clients: those live outside the
# redisbench cgroup, so cgroup.kill would otherwise leave them running.
cleanup_scenario() {
    stop_swap_logger
...
run_instance_scenario() {
    ...
    # Cleanup — also reap any stray memtier_benchmark clients that live outside
    # the cgroup so the next sweep point starts from a clean environment.
    pkill -f memtier_benchmark 2>/dev/null || true
    pkill -f redis-server 2>/dev/null || true
    echo 1 | sudo tee "$cg/cgroup.kill" >/dev/null 2>/dev/null || true
    sleep 1
