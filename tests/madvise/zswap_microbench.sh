...
for profile in "${compressor_list[@]}"; do
    ...
    baseline_result=$(ZPOOL_SOURCE=zswap ./madvise_test "$DATASET" "$NPGS_PER_THREAD" 2>&1)
    ...
    if (( BASELINE_ONLY == 0 )); then
        for nthreads in "${THREAD_LIST[@]}"; do
            result=$(ZPOOL_SOURCE=zswap ./madvise_test_mt "$DATASET" "$NPGS_PER_THREAD" "$nthreads" 2>/dev/null)
