...
# zswap_microbench.sh - Measure zswap per-page latency under varying concurrency
...
#
# Usage:
#   sudo ./zswap_microbench.sh [options]
#
# Options:
...
#   --log-mode, -l <mode>           Logging mode: none, zswap, ebpf (default: none)
...
Compressor profile naming: <algorithm>_r<reclaim_batchsize>_p<page_cluster>
  ...
  lzo-rle_r1_p3             lzo-rle CPU, batchsize=1, page_cluster=3
  zstd_r1_p3                zstd CPU, batchsize=1, page_cluster=3

Flow:
  For each compressor profile:
    1. Configure zswap compressor, reclaim_batchsize, page_cluster
    2. Run single-threaded baseline
    3. Run multi-threaded tests at each thread count
    4. Report per-page latency and zpool metrics
EOF
}
...
start_logging() {
    ...
    case "$mode" in
        none)
            ...
            ;;
        zswap)
            if [[ ! -x "${THIS_DIR}/../redis/zswap_log.sh" ]]; then
                handle_error "zswap_log.sh not found at ${THIS_DIR}/../redis/"
            fi
            "${THIS_DIR}/../redis/zswap_log.sh" 1 "$out_csv" > "$out_log" 2>&1 &
            LOG_PID=$!
            ;;
        ebpf)
...
# Configure zswap/kernel params for a compressor profile.
configure_compressor() {
    ...
    if [[ ! -x "${THIS_DIR}/../scripts/config_sys_zswap.sh" ]]; then
        handle_error "config_sys_zswap.sh not found or not executable at ${THIS_DIR}/../scripts/"
    fi
...
if [[ -z "$SZ" ]] || [[ "$SZ" -le 0 ]]; then
    handle_error "Dataset '$DATASET' appears empty or unreadable."
fi
NPGS=$(echo "scale=0; $SZ/4-1" | bc -l)
if [[ -z "$NPGS" ]] || [[ "$NPGS" -le 0 ]]; then
    handle_error "Failed to compute page count from dataset size ($SZ KB)."
fi
...
if [[ ! -f "madvise_test.c" ]]; then
    handle_error "Source file madvise_test.c not found in $(pwd)"
fi
if [[ ! -f "madvise_test_mt.c" ]]; then
    handle_error "Source file madvise_test_mt.c not found in $(pwd)"
fi
...
for profile in "${compressor_list[@]}"; do
    ...
    # Force zswap debugfs metrics for this benchmark.
    baseline_result=""
    if ! baseline_result=$(ZPOOL_SOURCE=zswap numactl --physcpubind=1 --localalloc ./madvise_test "$DATASET" "$NPGS_PER_THREAD" 2>&1); then
        echo "WARNING: madvise_test failed for '$profile' (exit code: $?) — skipping" >&2
        stop_logging
        continue
    fi
    ...
    if (( BASELINE_ONLY == 0 )); then
        for nthreads in "${THREAD_LIST[@]}"; do
            result=""
            if ! result=$(ZPOOL_SOURCE=zswap numactl --physcpubind="1-${nthreads}" --localalloc ./madvise_test_mt "$DATASET" "$NPGS_PER_THREAD" "$nthreads" 2>&1); then
                echo "WARNING: madvise_test_mt failed for '$profile' threads=$nthreads (exit code: $?)" >&2
                mt_out+="err "
                mt_in+="err "
                mt_out_sys+="err "
                mt_in_sys+="err "
